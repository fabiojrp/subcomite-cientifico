                ...
                SELECT TBL.REGIONAL_ID AS ID,
                    ...
                    FROM
                        (SELECT VACINACAO_MS.REGIONAL AS REGIONAL_ID,
                                ...
                                SUM(CASE WHEN VACINACAO_MS.VACINA_DESCRICAO_DOSE IN ('1ª Dose', '1ª Dose') THEN VACINACAO_MS.DOSES_APLICADAS END) AS D1,
                                SUM(CASE WHEN VACINACAO_MS.VACINA_DESCRICAO_DOSE IN ('2ª Dose', '2ª Dose', 'Única', 'Dose') THEN VACINACAO_MS.DOSES_APLICADAS END) AS D2
                            FROM VACINACAO_MS
        ...
        sql = """CREATE OR REPLACE VIEW PUBLIC.VIEW_VACINACAO_MS_RESUMO AS
                    ...
                    WHERE VACINACAO_MS.DATA =
                            (SELECT MAX(VACINACAO_MS.DATA) AS MAX_DATA
                                FROM VACINACAO_MS)
                        AND VACINACAO_MS.VACINA_DESCRICAO_DOSE IN ('2ª Dose', '2ª Dose','Dose','Única')
                    GROUP BY VACINACAO_MS.REGIONAL,
                        VACINACAO_MS.DATA
